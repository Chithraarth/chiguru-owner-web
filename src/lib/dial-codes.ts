// Flag is derived from the ISO 3166-1 alpha-2 code (two Regional Indicator
// Symbols), not hand-typed — avoids 195 hand-picked emoji going stale/wrong.
...
// All ITU E.164 country calling codes, one row per ISO 3166-1 country (a
// handful of dial codes are shared by several countries — e.g. +1 for
// NANP members, +7 for Russia/Kazakhstan — each still gets its own row so
// the country NAME in the list is always unambiguous, even though the code
// typed onto the phone number is the same). Sorted alphabetically by name.
